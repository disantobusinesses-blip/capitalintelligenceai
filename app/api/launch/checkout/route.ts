import Stripe from 'stripe'
import { NextRequest, NextResponse } from 'next/server'
import { TEMPLATES, CUSTOM_SITE_OPTIONS } from '@/lib/templates'

export async function POST(req: NextRequest) {
  try {
    const { templateId, tier, goLiveDate, siteType, customOption } = await req.json()

    const isCustom = siteType === 'custom'

    const template = isCustom ? null : TEMPLATES.find((t) => t.id === templateId)
    if (!isCustom && !template) {
      return NextResponse.json({ error: 'Invalid template' }, { status: 400 })
    }

    // Resolve the build tier (e.g. Basic / Premium) when the template offers them.
    const selectedTier =
      template?.tiers?.find((t) => t.id === tier) ?? template?.tiers?.[0] ?? null
    const templateName = template
      ? selectedTier && template.tiers && template.tiers.length > 1
        ? `${template.businessName} (${template.industry}), ${selectedTier.label}`
        : `${template.businessName} (${template.industry})`
      : ''

    // Custom builds that take a deposit are landing pages (Starter / Premium).
    // Multi-page sites are quote-only and never reach this checkout.
    const custom = isCustom
      ? CUSTOM_SITE_OPTIONS.find((o) => o.id === customOption && o.flow === 'deposit')
      : null
    if (isCustom && !custom) {
      return NextResponse.json({ error: 'Invalid custom build option' }, { status: 400 })
    }

    if (typeof goLiveDate !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(goLiveDate)) {
      return NextResponse.json({ error: 'Invalid go live date' }, { status: 400 })
    }

    const depositPriceId = process.env.STRIPE_PRICE_DEPOSIT
    if (!process.env.STRIPE_SECRET_KEY || !depositPriceId) {
      return NextResponse.json(
        { error: 'Payment is not configured. Please contact us.' },
        { status: 500 }
      )
    }

    const stripe = new Stripe(process.env.STRIPE_SECRET_KEY)
    const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://intelligentaisystem.com'

    const successParams = new URLSearchParams({
      template: isCustom ? custom!.id : template!.id,
      goLiveDate,
    })
    if (!isCustom && selectedTier && template!.tiers && template!.tiers.length > 1) {
      successParams.set('tier', selectedTier.id)
    }

    // Payment mode: we only charge the one-time $200 deposit now. Hosting is
    // billed separately once the site goes live, so it is not part of checkout.
    const session = await stripe.checkout.sessions.create({
      mode: 'payment',
      line_items: [{ price: depositPriceId, quantity: 1 }],
      success_url: `${baseUrl}/launch/onboarding?${successParams.toString()}&session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${baseUrl}/launch`,
      metadata: {
        flow: 'launch_my_site',
        siteType: isCustom ? 'custom' : 'template',
        templateId: isCustom ? custom!.id : template!.id,
        templateName: isCustom ? `${custom!.label} (${custom!.range})` : templateName,
        tier: isCustom ? '' : (selectedTier?.id ?? ''),
        goLiveDate,
      },
      billing_address_collection: 'required',
      phone_number_collection: { enabled: true },
    })

    return NextResponse.json({ url: session.url })
  } catch (err) {
    console.error('Launch checkout error:', err)
    return NextResponse.json(
      { error: 'Unable to start checkout. Please try again.' },
      { status: 500 }
    )
  }
}
